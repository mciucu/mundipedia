            ...
            : n === 2 ? hemispheres(points[0], points[1])
                : points.map(function (_, i) {
                    let cell = [],
                        ...
                        e = e0;
                    if (!e) return null;
                    let centre0 = e.triangle.centre;
                    do {
                        let centre = e.triangle.centre;
                        if (dot(centre, centre0) < ε - 1) {
                            let a = cartesian(points[e.neighbor.p.i]), b = cartesian(points[e.p.i]),
...
function hemispheres(a, b) {
    let c = geoInterpolate(a, b)(.5),
        n = cross(cross(cartesian(a), cartesian(b)), cartesian(c)),
        m = 1 / norm(n);
    n[0] *= m, n[1] *= m, n[2] *= m;
    let ring = geoCircle().origin(spherical(n))().coordinates[0];
    return [
...
d3.convexhull3d = function (points) {
    ...
    for (i = 4; i < n; ++i) {
        ...
        do {
            let t = a.t;
            for (let j = 0, m = t.visible.length; j < m; ++j) t.visible[j].remove();
            t.visible.length = 0;
...
export let geoVoronoi = d3.geo.voronoi;
